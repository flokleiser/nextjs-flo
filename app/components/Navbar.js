...
<motion.svg xmlns="http://www.w3.org/2000/svg" 
       ...
       animate={{ opacity: showSubNavbar? 1:0, 
        height: showSubNavbar? '8rem' : '4rem'}}
       transition={{duration: 0.3, delay: showSubNavbar? 0:0.3}} 
...
{/* <AnimatePresence mode="wait"> */}
        ...
        <AnimatePresence>
          {showNavbar && (
            ...
            >
              <nav className="flex flex-wrap justify-start items-center w-full h-full z-9990">
                {navItems.map((item) => {
                  return (
                      ...
                      return (
                        <motion.div key={subItem.path} whileTap={{ scale: 0.8 }}>
                          <Link
                            ...
                            onMouseLeave={() => setHoveringPath(false)}
                            onPointerDown={() => setActivePath(subItem.path)}
                            onClick={() => setShowSubNavbar(false)}
